...
**[Report a vulnerability](https://github.com/BitKiln/GitBound/security/advisories/new)**. Please do not open
a public issue for an unfixed security problem.

Include the GitBound version (`gitbound --version`), your operating system,
...
GitBound is a local tool. It runs as the user who invokes it and holds no
...
- **Non-destructive edits.** GitBound refuses to replace a hook or a directory
...
first run. Every published asset — desktop bundles and the standalone CLI, on
all three platforms — is accompanied by a `.sha256` file and by a build
provenance attestation generated by `actions/attest-build-provenance`. Verify
the download before running it:

```console
sha256sum --check GitBound_1.0.0_windows_x86_64.exe.sha256
```

```console
(Get-FileHash .\GitBound_1.0.0_windows_x86_64.exe -Algorithm SHA256).Hash
```

Provenance can be checked with the GitHub CLI:

```console
gh attestation verify GitBound_1.0.0_windows_x86_64.exe --repo BitKiln/GitBound
```
